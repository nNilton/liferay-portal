...
public class ReadyRestController extends BaseRestController {
	...
	private boolean _remove() {
		for (int page = 1;; page++) {
			JSONObject jsonObject = new JSONObject(get(_getLiferayAuthorization(), "https://learn-uat.liferay.com/o/c/p2s3knowledgearticles/scopes/guest?page="+ page+"&pageSize=500"));

			JSONArray items = jsonObject.getJSONArray("items");

			for (int i = 0; i < items.length(); i++) {
				JSONObject jsonObject1 = items.getJSONObject(i);
				delete(_getLiferayAuthorization(), "", "https://learn-uat.liferay.com/o/c/p2s3knowledgearticles/"+ jsonObject1.getLong("id"));
			}
	...
	private JSONObject _getKnowledgeArticleJSONObject(
		...
		try {
			JSONArray articleAttachmentsJSONArray = new JSONObject(
				get(
					_getZendeskAuthorization(),
					"https://liferay-support.zendesk.com/api/v2/help_center/articles/" +
	...
	private String _getLiferayAuthorization() {
		return _liferayOAuth2AccessTokenManager.getAuthorization(
			"liferay-learn-etc-cron-2-oauth-application-headless-server");
	}
	...
	@Value("${liferay.learn.dxp.document.folder.id}")
